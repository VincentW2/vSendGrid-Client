...
def set_csv_file():
    new_csv = input("Enter new CSV file path (can be absolute or relative): ").strip()
    if not new_csv:
        print("No file name entered. CSV file not changed.")
        return
    if not os.path.exists(new_csv):
        print(f"File '{new_csv}' does not exist. Please check the path and try again.")
        return
    SETTINGS["csv_file"] = new_csv
    with open(SETTINGS_FILE, 'w') as f:
        json.dump(SETTINGS, f, indent=2)
    print(f"CSV file updated to: {new_csv}\n")
    global CSV_FILE
    CSV_FILE = new_csv
...
def print_email_list():
    ...
    print(f"\nCurrent email list from '{campaign.csv_file}': (showing up to 5)")
...
def main():
    ...
    while True:
        print("Options:")
        print("1. Run campaign (send up to 5000 emails)")
        print("2. Check campaign status")
        print("3. Run custom batch size")
        print("4. Send test email to a specific address")
        print("5. Print current email list (first 5)")
        print("6. Change CSV file name/path")
        print("0. Exit")
        ...
        if choice == "1":
            ...
            print(f"   🕒 Last run: {stats['last_run']}")
            if stats['emails_remaining'] > 0:
                print(f"\n💡 Ready to send to {min(5000, stats['emails_remaining'])} more recipients!")
            else:
                print("\n🎉 Campaign complete!")
        elif choice == "3":
            try:
            ...
            test_email = input("Enter recipient email for test: ").strip()
            if '@' not in test_email or '.' not in test_email:
                print("Invalid email address.")
            else:
                send_campaign_email(test_email, show_output=True)
        elif choice == "5":
            print_email_list()
        elif choice == "6":
            set_csv_file()
